...
[script]
[windows]
fast-cargo subcommand *cargo_args:
...
clippy-fast *cargo_args:
    just fast-cargo clippy {{ cargo_args }}

# Build and run with Cranelift. Additional arguments are passed to Cargo.
run-fast *cargo_args:
    just fast-cargo run {{ cargo_args }}

# Build the DataTable example.
datatable-build-fast:
    just build-fast --package datatable

# Build and run the DataTable example.
datatable-fast:
    just run-fast --package datatable

# Build the native Viewer.
viewer-build-fast:
    just build-fast --package re_viewer --bin viewer

# Build and run the native Viewer.
viewer-fast:
    just run-fast --package re_viewer --bin viewer
